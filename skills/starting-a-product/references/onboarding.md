...
2. **Scaffold** once answered:
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/skills/starting-a-product/scripts/scaffold.mjs" --product "<name>" --lifecycle <x>
   ```
...
3. **Route** back to SKILL.md's process flow: existing → sunoku:writing-the-prd (existing
   mode); validate → sunoku:researching (validation mode); committed → sunoku:writing-the-prd
   (create mode).
