...
| `sunoku:viewing-the-record` | User or model | "Show tasks", "task board", "open the backlog". Serves a live read-only view of tasks + decisions on a local port (127.0.0.1, key-gated); open tabs reload as the record changes, and the server stops itself after ~15 idle minutes. |
