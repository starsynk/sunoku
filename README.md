...
| `sunoku:viewing-the-record` | User or model | "Show tasks", "task board", "open the backlog". Serves a live read-only view of tasks + decisions on a local port (127.0.0.1, key-gated); archived milestones sit under an Archive tab; open tabs reload as the record changes, and the server stops itself after ~15 idle minutes. |
| `sunoku:pruning-the-record` | User command only (model won't self-invoke) | Prune the record: fully-done milestones are archived in place (visible in the viewer's Archive tab, reversible via `--unarchive-milestone`); resolved decisions already absorbed into the PRD and superseded research files are deleted — git is their archive. Evidence shown, one confirm. Change Log never touched. |
