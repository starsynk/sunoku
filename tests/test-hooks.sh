...
check "ss: live+tracking cues track" 0 "sunoku:track" $? "$OUT"
check "ss: cue names read" 0 "sunoku:read" $? "$OUT"
check "ss: cue forbids auto-track" 0 "never auto-track" $? "$OUT"

D="$(mktemp -d)"; mkrepo "$D"
sed -i.bak 's/"tracking": true/"tracking": false/' "$D/.sunoku/status.json"
OUT="$(printf '%s' "$STDIN_MAIN" | CLAUDE_PROJECT_DIR="$D" node "$SS")"
check "ss: muted says muted" 0 "muted" $? "$OUT"

D="$(mktemp -d)"; mkrepo "$D"
sed -i.bak 's/"lifecycle": "live"/"lifecycle": "defining"/' "$D/.sunoku/status.json"
OUT="$(printf '%s' "$STDIN_MAIN" | CLAUDE_PROJECT_DIR="$D" node "$SS")"; check "ss: not live silent" 0 "" $? "$OUT"
